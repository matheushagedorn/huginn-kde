...
Item {
    ...
    // Empty until something says otherwise. It used to start on one specific
    // picture from one specific theme, which is what the shell painted for the
    // first moments of every start before the real wallpaper arrived and faded
    // over it. WallpaperService fills this in before the first frame.
    property string wallpaperPath: ""

    readonly property var themeCategories: [
        "All", "From wallpaper", "Dracula Pro", "Everblush", "Gruvbox", "Rosé Pine", "Catppuccin", "Everforest", "Tokyo Night", "Nord", "Solarized", "One Theme", "Monokai", "Cyberpunk"
    ]

    readonly property var variants: [

        // Colour taken from the wallpaper itself, the way every Material You
        // shell does it. `dynamic` means setVariant reads the picture instead
        // of these values, and leaves the wallpaper alone rather than swapping
        // in the one that belongs to a named theme.
        { category: "From wallpaper", name: "From wallpaper", dynamic: true, isDark: true, accent: "#9580ff", subAccent: "#ff80bf", bg: "#22212c", surface: "#2b2938", currentLine: "#454158", fg: "#f8f8f2", comment: "#7970a9" },
    ...
    // One line inside a monitoring card, text or bar. Fixed so that the second
    // row of one card is level with the second row of the card next to it:
    // letting each line take its natural height is what left the grid ragged.
    readonly property int cardSlot: 24
    ...
    // Hover inside the bar and the dock. `stateHover` is a grey lift, which
    // left the whole shell reading as colourless: the accent only ever showed
    // up on something already open. This is the same lift carrying the accent,
    // so pointing at a control is the first place the palette appears.
    readonly property color stateAccentHover: Qt.rgba(accent.r, accent.g, accent.b, isDark ? 0.16 : 0.12)
    ...
    // Reading colours for the monitors.
    //
    // Each card used to carry its own thresholds inline, so the CPU bar went
    // red past 80% while the GPU bar next to it stayed accent at 99%, and a
    // 76°C GPU read as a warning while a 79°C CPU did not. A reading is calm
    // until `warn`, warm from there, and alarming past `crit`; a card that
    // needs a different scale (a disk is not busy at 60% full) passes its own
    // numbers instead of hardcoding colours.
    function loadColor(pct, warn, crit) {
        var w = warn === undefined ? 60 : warn
        var c = crit === undefined ? 85 : crit
        if (pct >= c) return danger
        if (pct >= w) return warning
        return accent
    }

    function tempColor(deg, warn, crit) {
        var w = warn === undefined ? 75 : warn
        var c = crit === undefined ? 85 : crit
        if (deg >= c) return danger
        if (deg >= w) return warning
        return fg
    }
    ...
    function setVariant(name, isStartupRestoration) {
        for (let i = 0; i < variants.length; i++) {
            ...
            if (v.name === name) {
                ...
                if (v.dynamic) {
                    // The wallpaper is the input here, so it must not be
                    // replaced by the one a named theme would bring.
                    root.applyCachedPalette()
                    root.readWallpaperPalette()
                } else if (!isStartupRestoration) {
                ...
                Quickshell.execDetached([
                    "python3", Quickshell.env("HOME") + "/.config/huginn/services/python/theme_sync.py",
                    "--bg", v.bg,
                    "--surface", v.surface,
                    "--currentLine", v.currentLine,
    ...
    //
    // Runs the extractor on whatever picture is on screen and applies what it
    // finds. Everything downstream already reads these properties, so the bar,
    // the popups, the cards and the dock follow without knowing where the
    // numbers came from; theme_sync carries the same palette out to GTK, Qt
    // and the terminals.
    function readWallpaperPalette() {
        // Through a timer, not straight into the process: a wallpaper change
        // arrives as several property writes in a row, and starting a process
        // that is already running silently does nothing, which used to leave
        // the palette on whichever picture won the race.
    ...
    Timer {
        ...
        onTriggered: {
            ...
            let raw = String(root.wallpaperPath).replace("file://", "")
            if (!raw) return
            paletteProc.command = ["python3", Quickshell.env("HOME") + "/.config/huginn/services/python/wallpaper_palette.py", raw]
            paletteProc.running = true
        }
    }

    // Last derived palette, read before the first frame.
    //
    // Deriving takes a few hundred milliseconds, and until it answered the
    // shell painted itself in the fallback colours and then jumped to the real
    // ones. The cache is only trusted while it names the wallpaper actually on
    // screen; anything else is stale and the extractor's answer replaces it.
    FileView {
        id: paletteCache
        path: Quickshell.env("HOME") + "/.config/huginn_wallpaper_palette.json"
        blockLoading: true
        printErrors: false
    }

    function applyCachedPalette() {
        if (currentVariant !== "From wallpaper") return
        try {
            let cached = JSON.parse(paletteCache.text())
            let wanted = String(wallpaperPath).replace("file://", "")
            if (!cached || !cached.source || cached.source !== wanted) return
            applyPalette(cached)
        } catch (e) {}
    }
    ...
    function applyPalette(p) {
        ...
        root.subAccent = p.subAccent
        root.bg = p.bg
        root.surface = p.surface
        root.currentLine = p.currentLine
        root.selection = p.currentLine
        root.glassBg = p.bg
        root.glassBorder = p.currentLine
            ...
            onRead: data => {
                try {
                    ...
                    Quickshell.execDetached([
                        ...
                        "--fg", p.fg,
                        "--accent", p.accent,
                        "--subAccent", p.subAccent,
                        "--isDark", "true",
                        "--variantName", "From wallpaper"
                    ])
                } catch (e) {}
            }
        }
    ...
    // A new wallpaper while this variant is on means a new palette.
    onWallpaperPathChanged: {
        if (currentVariant === "From wallpaper") readWallpaperPalette()
    }
