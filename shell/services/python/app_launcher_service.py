    ...
    for d in desktop_dirs:
        ...
        try:
            for entry in sorted(os.scandir(d), key=lambda e: e.name):
                ...
                # A web app's icon is whatever the browser rendered from the
                # site; the theme usually has a better one under the app's name.
                icon = icon_theme.prefer_themed(icon, name)
                apps.append({
                    'id': desktop_id,
                    'name': name,
                    'exec': exec_cmd,
                    'icon': icon,
                    # How big to draw it so it sits at the same optical size as
                    # its neighbours; see icon_metrics.
                    'iconScale': icon_metrics.optical_scale(icon),
                    'categories': cats,
