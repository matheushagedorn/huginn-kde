...
# window decorations kept the colours of the previous theme until the next
# login.
...
def sync_kde(bg, surface, current_line, fg, accent, sub_accent, is_dark, variant_name):
    ...
    base_name = re.sub(r'[^a-zA-Z0-9]', '', variant_name)
    color_scheme_dir = os.path.expanduser('~/.local/share/color-schemes')
    os.makedirs(color_scheme_dir, exist_ok=True)

    # plasma-apply-colorscheme refuses to re-apply a scheme that is already
    # selected: "already set as the theme for the current Plasma session". The
    # file had been rewritten with new colours, but the name had not changed,
    # so nothing propagated. kdeglobals kept the colours of whichever theme
    # last changed the *name*, and every KDE and GTK app followed that: window
    # decorations, Brave's and Ghostty's own title bars, all stuck on an old
    # palette while the shell moved on. This bites hardest on the wallpaper
    # theme, whose name never changes at all.
    #
    # So the scheme alternates between two names. Plasma sees a real change
    # every time, rewrites kdeglobals in full and notifies every app, and
    # kde-gtk-config regenerates the GTK colours off the back of it. The
    # name that is no longer in use is deleted, so System Settings never shows
    # two copies of the same theme.
    current = ""
    try:
    ...
    alt_name = f"{base_name}2"
    clean_name = alt_name if current == base_name else base_name
    retired_name = base_name if clean_name == alt_name else alt_name
...
"""
    try:
        ...
        retired_file = os.path.join(color_scheme_dir, f"{retired_name}.colors")
...
Item {{
    ...
    // Dynamic Theme Colors (Auto-generated by Huginn theme engine)
...
"""
        ...
        user_splash_dir = os.path.expanduser('~/.local/share/plasma/look-and-feel/org.huginn.splash/contents/splash')
        ...
        meta_file = os.path.expanduser('~/.local/share/plasma/look-and-feel/org.huginn.splash/metadata.json')
        if not os.path.exists(meta_file):
            with open(meta_file, 'w', encoding='utf-8') as f:
                f.write('''{
    "KPlugin": {
        "Authors": [{"Email": "antigravity@google.com", "Name": "Huginn"}],
        "Description": "Dynamic theme-following splash screen for KDE Plasma 6",
        "Id": "org.huginn.splash",
        "License": "GPLv2+",
        "Name": "Huginn Dynamic Splash",
        "ServiceTypes": ["KPackage/Package"],
        "Version": "1.0"
    }
}''')

        repo_splash_dir = os.path.expanduser('~/.config/huginn/splash/org.huginn.splash/contents/splash')
        os.makedirs(repo_splash_dir, exist_ok=True)
        with open(os.path.join(repo_splash_dir, 'Splash.qml'), 'w', encoding='utf-8') as f:
            f.write(splash_qml_content)

        subprocess.run(["kwriteconfig6", "--file", "ksplashrc", "--group", "KSplash", "--key", "Theme", "org.huginn.splash"], capture_output=True)
...
def sync_gtk(bg, surface, current_line, fg, accent, is_dark):
    css_content = f"""/* Auto-generated by Huginn theme engine */
...
"""
    for dir_path in [os.path.expanduser('~/.config/gtk-3.0'), os.path.expanduser('~/.config/gtk-4.0')]:
        ...
        # kde-gtk-config writes colors.css and window_decorations.css next to
        # this file and expects gtk.css to import them. Overwriting gtk.css
        # dropped those imports, and with them the whole Breeze-GTK palette:
        # apps that draw their own title bar (Brave, Ghostty) kept painting it
        # in the colours of whichever theme was active the last time that
        # service ran. The imports come first so the definitions below win.
        header = ""
        for extra in ('colors.css', 'window_decorations.css'):
...
def sync_discord(bg, surface, current_line, fg, accent, sub_accent, is_dark):
    import json
    css_content = f"""/* Auto-generated by Huginn theme engine (Catppuccin Discord Architecture) */
...
"""
    ...
    for base_dir in discord_dirs:
        try:
            # 1. Write huginn-theme.css to themes/ directory
            themes_dir = os.path.join(base_dir, 'themes')
            os.makedirs(themes_dir, exist_ok=True)
            for theme_filename in ['huginn-theme.css', 'huginn.theme.css', 'huginn.css']:
            ...
            # 3. Enable huginn-theme.css and QuickCSS in settings.json
            for s_path in [
                ...
                try:
                    ...
                    if not isinstance(enabled_themes, list):
                        enabled_themes = []
                    if 'huginn-theme.css' not in enabled_themes:
                        enabled_themes.append('huginn-theme.css')
...
def sync_zen(bg, surface, current_line, fg, accent, sub_accent, is_dark):
    import subprocess
    chrome_css = f"""/* Auto-generated by Huginn theme engine */
:root {{
...
def slugify(name):
    ...
    return f"huginn-{s}" if s else "huginn"
...
def make_feishin_json(bg, surface, current_line, fg, accent, is_dark, name="Huginn", slug="huginn"):
...
def sync_feishin(bg, surface, current_line, fg, accent, sub_accent, is_dark, variant_name="Pro"):
    ...
    qs_json = make_feishin_json(bg, surface, current_line, fg, accent, is_dark, "Huginn", "huginn")
    ...
    for base_dir in feishin_dirs:
        try:
            for t_dir_name in ['Themes', 'themes']:
                ...
                # Write active variant JSON with runtime parameters + huginn.json
                active_path = os.path.join(themes_dir, f"{active_slug}.json")
                with open(active_path, 'w', encoding='utf-8') as f:
                    json.dump(active_json, f, indent=2)

                with open(os.path.join(themes_dir, "huginn.json"), 'w', encoding='utf-8') as f:
...
def sync_starship(bg, surface, current_line, fg, accent, sub_accent, is_dark, variant_name=""):
    ...
    starship_config = f"""# Starship Dynamic Theme (Auto-synced with Huginn)
...
def sync_vim(bg, surface, current_line, fg, accent, sub_accent, is_dark):
    ...
    vim_theme_content = f"""" Auto-generated by Huginn theme engine
highlight clear
if exists("syntax_on")
  syntax reset
endif
let g:colors_name = "huginn_theme"
...
"""
    ...
    with open(os.path.join(colors_dir, 'huginn_theme.vim'), 'w') as f:
...
def sync_btop(bg, surface, current_line, fg, accent, sub_accent):
    ...
    theme_content = f"""# Auto-generated by Huginn theme engine
theme[main_bg]="{bg}"
theme[main_fg]="{fg}"
theme[title]="{fg}"
theme[hi_fg]="{accent}"
...
"""
    try:
        with open(os.path.join(themes_dir, 'huginn.theme'), 'w') as f:
            ...
            if 'color_theme =' in conf:
                ...
                for l in lines:
                    if l.strip().startswith('color_theme ='):
                        new_lines.append('color_theme = "huginn"')
                    else:
                        new_lines.append(l)
                conf = '\n'.join(new_lines) + '\n'
            else:
                conf = 'color_theme = "huginn"\n' + conf
...
def sync_ghostty(bg, surface, current_line, fg, accent, sub_accent, is_dark=True, variant_name=""):
    ...
    config_content = f"""# Auto-generated by Huginn theme engine
background = {bg}
foreground = {fg}
selection-background = {current_line}
selection-foreground = {fg}
cursor-color = {accent}
# Ghostty draws its own title bar, so without this it takes the colour from
# the GTK theme and stays on the old palette while everything around it moves.
# "ghostty" means the window chrome uses the terminal's own colours.
...
"""
    ...
    # A running Ghostty reads its config once. SIGUSR2 is its reload signal,
    # so the windows already open follow the palette instead of waiting for
    # the next time the user opens a terminal.
    try:
        import subprocess
        subprocess.run(["pkill", "-USR2", "-x", "ghostty"], capture_output=True, timeout=5)
    except Exception:
        pass
...
"""
    try:
        with open(os.path.join(colors_dir, 'huginn.micro'), 'w') as f:
            f.write(colorscheme)
    except Exception:
        pass
    ...
    if os.path.exists(settings_file):
        ...
        try:
            import json
            with open(settings_file, 'w') as f:
                json.dump({"colorscheme": "huginn"}, f, indent=4)
        except Exception:
            pass
...
[General]
Description=Huginn Dynamic Theme
Opacity=1
Wallpaper=
"""
    try:
        with open(os.path.join(konsole_dir, 'Huginn.colorscheme'), 'w') as f:
            f.write(colorscheme)

        # Deploy Huginn.profile
...
"""
        with open(os.path.join(konsole_dir, 'Huginn.profile'), 'w') as f:
            f.write(profile_content)

        # Set Quickshell.profile as default in ~/.config/konsolerc
        subprocess.run(["kwriteconfig6", "--file", "konsolerc", "--group", "Desktop Entry", "--key", "DefaultProfile", "Huginn.profile"], capture_output=True)

        # Signal live open Konsole sessions over D-Bus to switch to the Huginn profile immediately
                ...
                try:
                    ...
                    for s_line in sessions.splitlines():
                        if "/Sessions/" in s_line:
                            s_path = s_line.strip()
                            subprocess.run([QDBUS, svc, s_path, "org.kde.konsole.Session.setProfile", "Huginn"], capture_output=True)
                except Exception:
                    pass
    except Exception:
        pass
...
def sync_xresources(bg, surface, current_line, fg, accent, sub_accent, is_dark=True, variant_name=""):
    ...
    xres_content = f"""! Auto-generated by Huginn theme engine
*.background: {bg}
*.foreground: {fg}
*.cursorColor: {accent}
*.color0: {bg}
...
def sync_obsidian(bg, surface, current_line, fg, accent, sub_accent, is_dark):
    ...
    obsidian_css = f"""/* Auto-generated by Huginn theme engine */
...
"""
    ...
    # Apply huginn-theme.css and update appearance.json to enable the snippet automatically
    for obs_dir in vault_dirs:
        try:
            snippets_dir = os.path.join(obs_dir, 'snippets')
            os.makedirs(snippets_dir, exist_ok=True)

            css_path = os.path.join(snippets_dir, 'huginn-theme.css')
            ...
            if 'huginn-theme' not in enabled_snippets:
                enabled_snippets.append('huginn-theme')
...
# GTK, KDE and Xresources are session-wide rather than per-app, so they always run.
always = True


def present(*candidates):
    """True when any candidate is a binary on PATH or an existing config path."""
...
def main():
    ...
    c = (args.bg, args.surface, args.currentLine, args.fg, args.accent, args.subAccent)

    # Each entry: (label, function, arguments, "is this app even here?").
    # Every sync used to run on every theme change, so switching themes created
    # ~/.config directories for editors, terminals and chat clients the user had
    # never installed. An app is considered present when its binary is on PATH
    # or it already has a config directory.
    targets = [
        ('gtk',       sync_gtk,        (*c[:5], is_dark),                    always),
        ('kde',       sync_kde,        (*c, is_dark, args.variantName),      always),
        ('alacritty', sync_alacritty,  (*c, is_dark, args.variantName),      present('alacritty', '~/.config/alacritty')),
        ('discord',   sync_discord,    (*c, is_dark),                        present('discord', '~/.config/discord', '~/.config/Vencord', '~/.config/BetterDiscord')),
        ('vscode',    sync_vscode,     (*c, is_dark, args.variantName),      present('code', '~/.config/Code', '~/.config/VSCodium')),
        ('zen',       sync_zen,        (*c, is_dark),                        present('zen-browser', '~/.zen')),
        ('feishin',   sync_feishin,    (*c, is_dark, args.variantName),      present('feishin', '~/.config/feishin')),
        ('obsidian',  sync_obsidian,   (*c, is_dark),                        present('obsidian', '~/.config/obsidian')),
        ('starship',  sync_starship,   (*c, is_dark, args.variantName),      present('starship', '~/.config/starship.toml')),
        ('vim',       sync_vim,        (*c, is_dark),                        present('vim', 'nvim', '~/.vim', '~/.config/nvim')),
        ('btop',      sync_btop,       c,                                    present('btop', '~/.config/btop')),
        ('fastfetch', sync_fastfetch,  (*c, is_dark, args.variantName),      present('fastfetch', '~/.config/fastfetch')),
        ('ghostty',   sync_ghostty,    (*c, is_dark, args.variantName),      present('ghostty', '~/.config/ghostty')),
        ('micro',     sync_micro,      c,                                    present('micro', '~/.config/micro')),
        ('konsole',   sync_konsole,    (*c, is_dark, args.variantName),      present('konsole', '~/.local/share/konsole')),
        ('xresources', sync_xresources, (*c, is_dark, args.variantName),     always),
    ]

    for label, fn, fn_args, enabled in targets:
        if not enabled:
            continue
        try:
            fn(*fn_args)
        except Exception as e:
            # One broken target must not stop the rest of the theme from applying.
            print(f'theme_sync: {label} failed: {e}', file=sys.stderr)
