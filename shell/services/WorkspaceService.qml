...
Item {
    ...
    function switchTo(index) {
        ...
        Quickshell.execDetached(["python3", Quickshell.env("HOME") + "/.config/huginn/services/python/workspace_service.py", "switch", index.toString()])
    }

    Process {
        id: queryProc
        command: ["python3", Quickshell.env("HOME") + "/.config/huginn/services/python/workspace_service.py"]
        running: true
        stdout: SplitParser {
            onRead: data => {
                try {
                    ...
                    if (parsed && typeof parsed === "object") {
                        ...
                        // Only when it actually changed: this is polled every
                        // 120ms, and assigning an equal array still rebuilds
                        // every delegate that reads it.
                        if (names.length !== root.workspaceNames.length
                            || names.some((n, i) => n !== root.workspaceNames[i])) {
