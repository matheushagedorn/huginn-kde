...
Item {
    ...
    function deleteEntry(id) {
        ...
        if (entry) {
            ...
            if (query) {
                ...
                deleteProc.command = ["python3", Quickshell.env("HOME") + "/.config/huginn/services/python/clipboard_service.py", "delete", query]
    ...
    function clearAll() {
        ...
        items = []
        itemsChanged()
        copyProc.command = ["bash", "-c", "cliphist wipe 2>/dev/null; wl-copy -c 2>/dev/null; python3 " + Quickshell.env("HOME") + "/.config/huginn/services/python/clipboard_service.py clear 2>/dev/null"]
        copyProc.running = true
    }
    ...
    Process {
        id: scanProc
        command: ["python3", Quickshell.env("HOME") + "/.config/huginn/services/python/clipboard_service.py"]
        running: false
        onExited: {
            scanProc.running = false
        }
