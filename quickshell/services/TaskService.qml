...
Item {
    ...
    function focusSpecificWindow(winObj, iconX, iconY) {
        ...
        proc.running = false
        proc.command = ["python3", Quickshell.env("HOME") + "/.config/quickshell/services/python/kwin_focus.py", query, ix.toString(), iy.toString(), winId]
        proc.running = true
    }
    ...
    function closeSpecificWindow(winObj) {
        ...
        Quickshell.execDetached(["python3", Quickshell.env("HOME") + "/.config/quickshell/services/python/kwin_close.py", winId, query])
    }
    ...
    function closeApp(appId) {
        if (appId) {
            ...
            proc.running = false
            proc.command = ["python3", Quickshell.env("HOME") + "/.config/quickshell/services/python/kwin_close.py", appId]
            proc.running = true
        }
    }
